...
class TriageBot(commands.Bot):
    ...
    async def on_ready(self):
        print("\n" + "="*50)
        print("ðŸ“© Intelligence Agent is ONLINE!")
    ...
    async def run_triage_cycle(self, ignore_history: bool = False, interaction: discord.Interaction = None):
        ...
        async def progress_hook(current, total):
            if interaction and (current == 1 or current % 5 == 0 or current == total):
                status_msg = f"ðŸ“© **Triaging email {current} of {total}...** (This takes a moment)"
...
class SensitivitySelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(f"âœ… **Notification Sensitivity set to:** {self.values[0]}+", ephemeral=True)
...
class ToneSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        with open(CONFIG_PATH, "w") as f:
            json.dump(config, f, indent=4)
        await interaction.response.send_message(f"ðŸŽ­ **AI Tone updated to:** {self.values[0]}", ephemeral=True)
...
class VIPModal(discord.ui.Modal, title='Manage VIP List'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        with open(CONFIG_PATH, "w") as f:
            json.dump(config, f, indent=4)
        await interaction.response.send_message(f"ðŸŒŸ **VIP List Updated!**", ephemeral=True)
...
class UrgencyModal(discord.ui.Modal, title='Manage Urgency Triggers'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        view = RescanView(bot)
        await interaction.response.send_message(f"ðŸš¨ **Urgency Triggers Updated!**", view=view)
...
class SettingsView(discord.ui.View):
    ...
    @discord.ui.button(label="ðŸ“ Toggle Auto-Filing", style=discord.ButtonStyle.secondary)
    async def toggle_filing(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        with open(CONFIG_PATH, "w") as f:
            json.dump(config, f, indent=4)
        button.label = f"ðŸ“ Auto-Filing: {'ON' if config['enable_filing'] else 'OFF'}"
        await interaction.response.edit_message(view=self)

    @discord.ui.button(label="ðŸŒŸ Update VIPs", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(label="ðŸš¨ Manage Triggers", style=discord.ButtonStyle.secondary)
...
class RescanView(discord.ui.View):
    ...
    @discord.ui.button(label="ðŸ” Re-Scan Latest Emails", style=discord.ButtonStyle.primary)
    async def rescan_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.edit_original_response(
            content=f"âœ… **Deep Sweep Complete.** Found {count} items that match your new rules."
        )
        self.stop()
...
async def settings(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title="ðŸ“© Intelligence Agent : Control Panel", description="Manage your analyst's behavior and sensitivity here.", color=0x3498DB)
    embed.add_field(name="Current Goal", value=f"_{config.get('goal', 'None')}_", inline=False)
    
    triggers = config.get("urgency", "None").split(',')
    trigger_list = "\n".join([f"â€¢ {t.strip()}" for t in triggers if t.strip()])
    embed.add_field(name="ðŸš¨ Active Urgency Triggers", value=trigger_list or "None", inline=False)
...
async def help_cmd(interaction: discord.Interaction):
    embed = discord.Embed(title="ðŸ“š Intelligence Agent Manual", color=0x3498DB)
    embed.description = (
        ...
        "### ðŸ” Features\n"
        "**Deep Sweep:** Use the [ðŸ” Re-Scan] button to check history after updating rules.\n"
...
async def update_goal(interaction: discord.Interaction, new_goal: str):
    ...
    bot.reload_orchestrator()
    view = RescanView(bot)
    await interaction.response.send_message(f"âœ… **Business Goal Updated:** {new_goal}", view=view)
...
async def update_urgency(interaction: discord.Interaction, triggers: str):
    ...
    bot.reload_orchestrator()
    view = RescanView(bot)
    await interaction.response.send_message(f"ðŸš¨ **Urgency Triggers Updated:** {triggers}", view=view)
...
async def sweep(interaction: discord.Interaction, rescan_latest: bool = False):
    ...
    await interaction.edit_original_response(content=f"âœ… **Sweep Complete.** Processed {count} items.")

if __name__ == "__main__":
    if not TOKEN:
        print("âŒ ERROR: TRIAGE_BOT_TOKEN not found in .env")
        sys.exit(1)
    bot.run(TOKEN)
